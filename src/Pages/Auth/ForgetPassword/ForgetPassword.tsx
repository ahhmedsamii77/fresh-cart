...
import { useForgetPassword } from "../../../lib/react-query/react-query";
import { ForgetPasswordType } from "./ForgetPassword.types";
export default function ForgetPassword() {
  const navigate = useNavigate();
  const { mutateAsync: fotgetpassword } = useForgetPassword();
  const initialValues: ForgetPasswordType = {
    email: "",
  }
  async function onSubmit(values: ForgetPasswordType) {
    try {
      await fotgetpassword(values);
      toast.success('reset code sent to your email', { duration: 1500 });
      navigate('/reset-code');
